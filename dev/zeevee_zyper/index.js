...
const POLL_PERIOD_FAST = 3000;           // Continuous polling function interval
...
exports.createDevice = base => {
  ...
  function setup(_config) {
    ...
    base.setPoll({ action: 'getDecoderStatus', period: POLL_PERIOD_FAST, enablePollFn: isConnected, startImmediately: false });

    // Initialise variables for each decoder
    for (let i = 0; i < config.decoders.length; i++) {
      const decoder = config.decoders[i];
      decoder.varname = `Sources_${decoder.name.replace(/[^A-Za-z0-9_]/g, '')}`;  // Make legal variable name
      base.createVariable({
  ...
  function getDecoderStatus() {
    sendDefer('show device status decoders\n');
  }
  ...
  return {
    ...
    getDeviceConfig, getMultiviews, getVideowalls, updateSources, getDecoderStatus,
    selectSource
  };
};