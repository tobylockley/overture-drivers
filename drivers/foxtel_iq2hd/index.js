...
exports.createDevice = base => {
  ...
  function setup(_config) {
    ...
    // Create commands enum variable
    base.createVariable({
      name: 'IR_Commands',
      type: 'enum',
      enums: ['Idle'].concat(Object.keys(ir_codes)),
      perform: {
        action: 'sendCommand',
        params: {
          Index: '$value'
  ...
  function onFrame(data) {
    ...
    if (pendingCommand && pendingCommand.action === 'sendCommand') {
      const cmdName = base.getVar('IR_Commands').enums[pendingCommand.params.Index]
