...
class DriverDevice {
    constructor(base) {
        ...
        this.buttonsConnected = this.base.getVar('ButtonsConnected');
    }
    // SETUP //
    setup(config) {
        this.config = config;
    ...
    setDisplayMode({ Name }) {
        ...
        this.sendRequest('v1.0/Display/Mode', 'PUT', { value: Name })
            .then(function () {
            self.base.getVar('DisplayMode').string = Name;
    ...
    getInfo() {
        ...
        this.sendRequest('v1.0/DeviceInfo/ModelName')
            ...
            return self.sendRequest('v1.0/Buttons/ButtonTable');
        })
            // Buttons Connected
            .then(function (buttonTable) {
            let buttonCount = 0
            for (let button of Object.values(buttonTable)) {
                if (button.Connected) buttonCount += 1
            }
            self.buttonsConnected.value = buttonCount
