...
function zipDriver(inputPath, outputPath, moveToArchive = true) {
    ...
    let archivePath = path.join(outputPath, 'archive')

    const pkg = require(path.join(inputPath, 'package.json'))
    if (!(pkg.name && pkg.version && pkg.overture))
        throw new Error(`${inputPath} does not contain an overture driver`)
    ...
    if (files && moveToArchive) {
        for (let file of files) {
            if (file !== zipFilePath) {
                // Make sure archive folder exists update
                try {
                ...
                fs.renameSync(file, path.join(archivePath, filename))
    ...
    outputStream.on('close', function() {
        console.log(
            `Driver files saved to ${zipFilePath} (${archive.pointer()} bytes)`
        )
    })
...
if (require.main === module) {
    if (process.argv.length < 4)
        throw new Error(
            'Please provide input path and output path as command line argument'
        )

    const driverPath = process.argv[2]
    if (!fs.statSync(driverPath).isDirectory())
        throw new Error(`${driverPath} must be a directory`)

    const zipPath = process.argv[3]
    if (!fs.statSync(zipPath).isDirectory())
        throw new Error(`${zipPath} must be a directory`)
