...
String.prototype.toTitleCase = function () {
  return this.replace( /\w\w+/g, txt => txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase() );
};
...
console.log('Before proceeding, you must place codes into raw.txt (copy from email)')
...
// Folder creation
const driver_name = `${brand}_${model}`.toLowerCase().replace(/[^0-9a-zA-Z_\-. ]/g, '')  // Make legal filename
...
while (match = regex.exec(data)) {
  ir_codes[match[1].toTitleCase()] = match[2]
}
fs.writeFileSync(path.join(outputPath, 'ir_codes.json'), JSON.stringify(ir_codes, null, 2))

// package.json
let pkg = JSON.parse(fs.readFileSync(path.join(__dirname, 'template_package.json')))
...
// index.js
fs.copyFileSync(path.join(__dirname, 'template_index.js'), path.join(outputPath, 'index.js'))

let zipPath = path.resolve(`${__dirname}/../../zip`)
require(path.resolve(`${__dirname}/../zip.js`))(outputPath, zipPath)  // Zip driver using zip script

console.log(`Complete! Driver saved in ${outputPath}, zip file saved in ${zipPath}/`)